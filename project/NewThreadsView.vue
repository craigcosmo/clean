       ...
       <template v-if="this.channel.only_shareholder_threads == 0">
        ...
        <div v-if="allitem">
            <div v-for="(thread, index) in items">
                <new-thread-view :key="thread.id" v-if="thread != null" :thread="thread" :expand="expand" :count="thread.likesCount" :iframe_height="'273'" :single_view="0" :no_comments="channel.no_comments" @deleted="remove(index)"></new-thread-view>
            </div>
